use super::targetting::find_item_position;
use crate::*;
...
pub fn item_trigger(creator: Option<Entity>, item: Entity, targets: &Targets, ecs: &mut World) {
    ...
    if !event_trigger(creator, item, targets, ecs) {
        return;
    }
...
pub fn spell_trigger(creator: Option<Entity>, spell: Entity, targets: &Targets, ecs: &mut World) {
    let mut targetting = targets.clone();
    ...
    if let Some(template) = ecs.read_storage::<SpellTemplate>().get(spell) {
        ...
        if let Some(caster) = creator {
            ...
            if ecs.read_storage::<AlwaysTargetsSelf>().get(spell).is_some() {
                if let Some(pos) = ecs.read_storage::<Position>().get(caster) {
                    let map = ecs.fetch::<Map>();
                    targetting = if let Some(aoe) = ecs.read_storage::<AreaOfEffect>().get(spell) {
                        Targets::Tiles {
        ...
        if ecs.read_storage::<SingleActivation>().get(spell).is_some() && creator.is_some() {
            self_destruct = true;
        }
    }
    event_trigger(creator, spell, &targetting, ecs);
    if self_destruct {
        ecs.entities()
